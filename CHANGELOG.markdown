...
- Added a Validation Handler, which allows building your own custom validation
  system. The LiveValidation system still exists as the
  `lv_validation_handler.erl`, but the new `default_validation_handler`, is
  _significantly_ simplified, relying on basic Nitrogen operations.
- Added a Secret Handler for retrieving secrets (API Keys, etc). The
  `default_secret_handler` is extremely basic and simply retrieves the secrets
  from a file in the user's `$HOME` directory. The contents of this file are
  cached and read using the new
  [`nitro_secret`](https://github.com/nitrogen/nitro_secret) dependency
  application.
- Added a Modal Handler for working with popup modal menus. This enables the
  use of rich popup modals and can be customized to use the modal system of
  whichever front-end framework you wish to use (e.g. Bootstrap, Bulma, etc). To
  go along with the new modal handler, the `#confirm{}` action has been reworked,
  and two new actions have been added: `#modal{}` and `#prompt{}`. All three are
  able to handle rich popups, with `#confirm{}` and `#prompt{}` being able to
  fall back to `window.confirm()` and `window.prompt()` versions by providing the
  `basic=true` attributes to the records.
- Added `#quickform{}` element, which makes building CRUD-style forms with many
  fields fast and easy without sacrificing flexibility.
...
- Added the `#icon` element, which provides an icon mechanism that integrates
  with FontAwesome, LineAwesome, Google Material Icons, Bootstrap Icons, and
  custom Icon Fonts from IcoMoon, as well as any other Icon font you might want
  to use.
- Update `#textbox` to properly support the date/time/datetime-local types.
- Added `/2` (arity-2) versions to conversion functions like `wf:to_integer`,
...
- Implemented a new handler system for grid systems and front-end frameworks.
  ...
  `nitrogen_core` application config. This works by overwriting the `module`
  attribute of the `#grid{}` elements, allowing you to specify a different module
  to process the `#grid{}` elements.
...
- Reworked the `#google_chart{}` element to use Google's newer API.
...
- Ensured support through OTP 29
